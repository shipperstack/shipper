...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('username', nargs=1, action='store', help='Username of user to be init')

    def handle(self, *args, **options):
        self.stdout.write("Initializing user {}. Do you want to continue? [y/N] ".format(options['username'][0]),
                          ending='')
        ...
        if input().lower() == 'y':
            ...
            self.stdout.write("The specified user has been added to all enabled devices!")
        else:
