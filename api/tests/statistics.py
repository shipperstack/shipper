...
class StatisticsIncrementTestCase(APITestCase):
    ...
    def test_v1_download_build_counter(self):
        previous_count = Build.objects.get(file_name="Bliss-v14-angler-OFFICIAL-vanilla-20200608").build_stats.count()
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(Build.objects.get(file_name="Bliss-v14-angler-OFFICIAL-vanilla-20200608").build_stats.count() > previous_count)
...
def mock_statistics_object_create(time, device, build, ip):
    new_stat = Statistics.objects.create(
        device=device,
        build=build,
        ip=ip
    )
