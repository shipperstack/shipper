...
class RegisterForm(forms.ModelForm):
    ...
    password_verify = forms.CharField(label=gettext_lazy('Verify Password'), widget=forms.PasswordInput)
    ...
    def clean_email(self):
        if User.objects.filter(email=self.cleaned_data['email']).exists():
            raise forms.ValidationError(gettext_lazy('This email address is already registered and cannot be used!'))
